...
If you want, I can:
1) add a small guard in `model.py` so missing `ShuffleNet` won't break imports, and
2) remove hard-coded `wandb.login(...)` lines and switch to reading `WANDB_API_KEY` from environment.

Tell me which (if any) code fixes you'd like me to apply and I will update the repo. 👇
